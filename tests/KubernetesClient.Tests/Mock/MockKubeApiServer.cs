...
namespace k8s.Tests.Mock
{
    ...
    internal class MockKubeApiServer : IDisposable
    {
        ...
        public MockKubeApiServer(
            ITestOutputHelper testOutput,
            Func<HttpContext, Task<bool>> shouldNext = default,
            Action<ListenOptions> listenConfigure = default,
            string resp = MockPodResponse)
        {
            _webHost = BuildWebHost(testOutput, shouldNext, listenConfigure, resp);
            _webHost.Start();
        }
        ...
        private async Task<bool> ShouldNext(HttpContext httpContext)
        {
            ...
            if (isWatch && !_serverFlags.HasFlag(MockKubeServerFlags.Throw500))
            {
                ...
                foreach (var serverFlag in Enum.GetValues(_serverFlags.GetType()))
                {
                    switch (serverFlag)
                    {
        ...
        private IWebHost BuildWebHost(
            ITestOutputHelper testOutput,
            Func<HttpContext, Task<bool>> shouldNext,
            Action<ListenOptions> listenConfigure,
            string resp)
        {
