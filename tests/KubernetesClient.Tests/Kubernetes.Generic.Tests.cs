...
namespace k8s.Tests
{
    public class Kubernetes_Generic_Tests : IClassFixture<KindCluster>
    {
        ...
        [Fact(Skip = "Integration Test")]
        public async Task KubernetesGenericMethodsSubresourcesIntegrationTest()
        {
            var namespaceName = Guid.NewGuid().ToString("N");
            var config = KubernetesClientConfiguration.BuildDefaultConfig();
            var client = new Kubernetes(config);
            var ns = new V1Namespace().Initialize();
            ns.Metadata.Name = namespaceName;

            // create
            var creationResult = await client.CreateWithHttpMessagesAsync(ns);

            creationResult.Response.EnsureSuccessStatusCode();

            // patch status
            var patch = new JsonPatchDocument<V1Namespace>();
            patch.Add(x => x.Status.Phase, "Active");

            var patchResult = await client.PatchWithHttpMessagesAsync<V1Namespace>(new V1Patch(patch), namespaceName, statusOnly: true);
            patchResult.Response.EnsureSuccessStatusCode();
        }
    }
}
